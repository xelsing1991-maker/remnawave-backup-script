...
load_or_create_config() {
    if [[ -f "$CONFIG_FILE" ]]; then
        ...
        S3_RETAIN_DAYS=$(normalize_retention_days "$S3_RETAIN_DAYS" 30)
        RETAIN_BACKUPS_DAYS=$(normalize_retention_days "$RETAIN_BACKUPS_DAYS" 7)
        BACKUP_INTERVAL_HOURS=$(normalize_backup_interval_hours "${BACKUP_INTERVAL_HOURS:-}")
        LANG_CODE=${LANG_CODE:-}
...
get_server_ip() {
    local public_ip
    public_ip=$(curl -4 -s --max-time 5 ${TG_PROXY:+--proxy "$TG_PROXY"} https://api.ipify.org 2>/dev/null | tr -d '[:space:]')
...
build_telegram_message_link() {
    ...
    message_id=$(echo "$response_body" | sed -n 's/.*"message_id":[[:space:]]*\([0-9]\+\).*/\1/p' | head -n 1)
    username=$(echo "$response_body" | sed -n 's/.*"username":"\([^"]*\)".*/\1/p' | head -n 1)
    chat_id=$(echo "$response_body" | sed -n 's/.*"chat":{[^}]*"id":[[:space:]]*\(-\?[0-9]\+\).*/\1/p' | head -n 1)
    chat_type=$(echo "$response_body" | sed -n 's/.*"chat":{[^}]*"type":"\([^"]*\)".*/\1/p' | head -n 1)
    ...
    if [[ -n "$username" && "$chat_type" != "private" ]]; then
        echo "https://t.me/${username}/${message_id}"
    elif [[ "$chat_id" == -100* ]]; then
        echo "https://t.me/c/${chat_id#-100}/${message_id}"
    else
...
normalize_backup_interval_hours() {
    local value="$1"
...
send_telegram_document() {
    ...
    if [[ -n "$parse_mode" && "$parse_mode" != "None" ]]; then
        form_params+=(-F parse_mode="$parse_mode")
    fi
    ...
    if [[ "$http_code" == "200" ]]; then
        LAST_TELEGRAM_MESSAGE_LINK=$(build_telegram_message_link "$response_body")
        return 0
    else
        echo -e "${RED}❌ $(t tg_api_err) ${BOLD}$http_code${RESET}. $(t tg_resp_label) ${BOLD}$response_body${RESET}. $(t tg_maybe_big)${RESET}"
        return 1
    fi
}

send_telegram_document_parts() {
    local file_path="$1"
    local caption="$2"
    local part_size_mb="${3:-38}"
    ...
    local part_size_bytes=$((part_size_mb * 1024 * 1024))
    local file_size_bytes
    file_size_bytes=$(stat -c%s "$file_path" 2>/dev/null || stat -f%z "$file_path" 2>/dev/null || echo "0")

    if [[ "$file_size_bytes" -le "$part_size_bytes" ]]; then
        send_telegram_document "$file_path" "$caption"
        return $?
    fi

    if ! command -v split >/dev/null 2>&1; then
        print_message "ERROR" "split command not found. Cannot split backup for Telegram upload."
        return 1
    fi

    local file_name
    file_name=$(basename "$file_path")
    local parts_dir="${file_path}.parts"
    local part_prefix="${parts_dir}/${file_name}.part-"

    rm -rf "$parts_dir"
    mkdir -p "$parts_dir" || return 1

    if ! split -b "${part_size_mb}M" -d -a 3 "$file_path" "$part_prefix"; then
        rm -rf "$parts_dir"
        return 1
    fi

    local total_parts
    total_parts=$(find "$parts_dir" -type f -name "${file_name}.part-*" | wc -l | tr -d ' ')
    ...
    local part_caption
    local parts_links=""
    local rebuild_cmd="cat ${file_name}.part-* > ${file_name}"

    for part_file in "$parts_dir"/"${file_name}.part-"*; do
        part_caption="${caption}"$'\n\n'"Part ${part_number}/${total_parts}"$'\n'"Rebuild: ${rebuild_cmd}"

        if ! send_telegram_document "$part_file" "$part_caption" "None"; then
            rm -rf "$parts_dir"
            return 1
        fi

        if [[ -n "$LAST_TELEGRAM_MESSAGE_LINK" ]]; then
            parts_links+=$'\n'"${part_number}. ${LAST_TELEGRAM_MESSAGE_LINK}"
        else
            parts_links+=$'\n'"${part_number}. link unavailable"
        fi
    ...
    local windows_instruction
    windows_instruction="🧩 Бэкап отправлен частями"$'\n'"📦 Файл: ${file_name}"$'\n'"🔢 Частей: ${total_parts}"$'\n'"🔗 Ссылки/ID частей:${parts_links}"$'\n\n'"🪟 Как собрать и распаковать в Windows:"$'\n'"1. Скачайте все части в одну папку."$'\n'"2. Откройте PowerShell в этой папке."$'\n'"3. Выполните: cmd /c copy /b ${file_name}.part-* ${file_name}"$'\n'"4. Распакуйте ${file_name} через 7-Zip/WinRAR или командой tar -xzf ${file_name}"
    send_telegram_message "$windows_instruction" "None" >/dev/null 2>&1
...
cleanup_local_old_backups() {
    local retain_days
    retain_days=$(normalize_retention_days "$RETAIN_BACKUPS_DAYS" 7)
    RETAIN_BACKUPS_DAYS="$retain_days"

    local retain_minutes=$((retain_days * 24 * 60))
    local deleted_count=0
    local item
    ...
    while IFS= read -r item; do
        ...
        find "$BACKUP_DIR" -maxdepth 1 \
            \( -type f -name "remnawave_backup_*.tar.gz" -o -type d -name "remnawave_backup_*.tar.gz.parts" \) \
            -mmin +"$retain_minutes" -print 2>/dev/null
    )

    find "$BACKUP_DIR" -maxdepth 1 \
        \( -type d -name "restore_temp_*" -o -type d -name "extract_temp_*" -o -type d -name "bot_extract_temp_*" \) \
        -mmin +1440 -exec rm -rf -- {} + 2>/dev/null || true

    if [[ $deleted_count -gt 0 ]]; then
        print_message "INFO" "Local old backup files removed: $deleted_count"
    fi
}

cleanup_interval_old_backups() {
    local interval_hours
    interval_hours=$(normalize_backup_interval_hours "$BACKUP_INTERVAL_HOURS")
    ...
    local interval_minutes=$((interval_hours * 60))
    local older_than_minutes=$((interval_minutes - 1))
    local deleted_count=0
    local item

    print_message "INFO" "Interval backup cleanup: removing local backups older than ${interval_hours} hour(s) before creating a new one..."
    ...
    while IFS= read -r item; do
        ...
        find "$BACKUP_DIR" -maxdepth 1 \
            \( -type f -name "remnawave_backup_*.tar.gz" -o -type d -name "remnawave_backup_*.tar.gz.parts" \) \
            -mmin +"$older_than_minutes" -print 2>/dev/null
    )

    if [[ $deleted_count -gt 0 ]]; then
        print_message "INFO" "Interval cleanup removed old local backup items: $deleted_count"
    fi
}
...
METAEOF
    ...
    if [[ -f "$BACKUP_DIR/$BACKUP_FILE_FINAL" ]]; then
        if [[ "$UPLOAD_METHOD" == "telegram" ]]; then
            ...
            local tg_part_size=$((38 * 1024 * 1024))
            if [[ "$file_size_bytes" -gt "$tg_part_size" ]]; then
                print_message "INFO" "Backup is larger than 38 MB. Splitting and sending to Telegram in parts..."
            fi

            if send_telegram_document_parts "$BACKUP_DIR/$BACKUP_FILE_FINAL" "$caption_text" 38; then
...
configure_interval_backup() {
    ...
    while true; do
        clear
        echo -e "${GREEN}${BOLD}$(t interval_title)${RESET}"
        echo ""

        if [[ -n "$BACKUP_INTERVAL_HOURS" ]]; then
            print_message "INFO" "$(t interval_current) ${BOLD}${BACKUP_INTERVAL_HOURS} $(t interval_hours)${RESET}"
            print_message "INFO" "$(t interval_keep_info) ${BOLD}${BACKUP_INTERVAL_HOURS} $(t interval_hours)${RESET}"
        else
            print_message "INFO" "$(t interval_disabled)"
        fi

        echo ""
        echo "   1. $(t interval_every_1)"
        echo "   2. $(t interval_every_3)"
        echo "   3. $(t interval_every_6)"
        echo "   4. $(t interval_every_12)"
        echo "   5. $(t interval_disable)"
        echo ""
        echo "   0. $(t back_to_menu)"
        echo ""

        local choice interval_hours cron_entry
        read -rp "${GREEN}[?]${RESET} $(t select_option)" choice
        echo ""

        case "$choice" in
            1) interval_hours=1; cron_entry="0 * * * *" ;;
            2) interval_hours=3; cron_entry="0 */3 * * *" ;;
            3) interval_hours=6; cron_entry="0 */6 * * *" ;;
            4) interval_hours=12; cron_entry="0 */12 * * *" ;;
            5)
                print_message "INFO" "$(t cron_disabling)"
                (crontab -l 2>/dev/null | grep -vF "$SCRIPT_PATH backup") | crontab -
                BACKUP_INTERVAL_HOURS=""
                CRON_TIMES=""
                save_config
                print_message "SUCCESS" "$(t interval_disabled_ok)"
                echo ""
                read -rp "$(t press_enter)"
                continue
                ;;
            0) break ;;
            *) print_message "ERROR" "$(t invalid_input_select)"; read -rp "$(t press_enter)"; continue ;;
        esac
        ...
        grep -vF "$SCRIPT_PATH backup" "$temp_crontab_file" > "$temp_crontab_file.tmp"
        mv "$temp_crontab_file.tmp" "$temp_crontab_file"
        echo "$cron_entry $SCRIPT_PATH backup >> /var/log/rw_backup_cron.log 2>&1" >> "$temp_crontab_file"

        if crontab "$temp_crontab_file"; then
            BACKUP_INTERVAL_HOURS="$interval_hours"
            CRON_TIMES="every ${interval_hours} hour(s)"
            save_config
            print_message "SUCCESS" "$(t interval_saved) ${BOLD}${interval_hours} $(t interval_hours)${RESET}"
            print_message "INFO" "$(t interval_logic)"
        else
            print_message "ERROR" "$(t cron_fail)"
        fi
...
main_menu() {
    while true; do
        ...
        echo "   5. $(t menu_interval_backup)"
        echo "   6. $(t menu_upload_method)"
        echo "   7. $(t menu_settings)"
        echo ""
        echo "   8. $(t menu_update)"
        echo "   9. $(t menu_remove)"
        echo ""
        echo "   0. $(t exit)"
        ...
        case $choice in
            ...
            4) setup_auto_send ;;
            5) configure_interval_backup ;;
            6) configure_upload_method ;;
